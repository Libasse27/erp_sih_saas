...
| `MembershipGranted` | `identity.membership.granted` | 1 | `UserTenantMembership.grant()`, via `GrantMembershipHandler` | `userId: string` | `tenant.seedFacilityConfigurationOnMembershipGranted` (module `tenant`, ADR-0008 §10, amendement 1, Phase 0 étape 10/13) — troisième étape chorégraphiée de la Saga de provisioning : sème la configuration technique minimale du tenant. Réagit à TOUT octroi de membership (pas seulement le premier `ADMIN_ETABLISSEMENT` du provisioning initial), sans conséquence car `SeedFacilityConfigurationHandler` est idempotent par tenant (voir le commentaire de tête du consommateur). N'extrait aucun champ du payload, seul `envelope.tenantId` est utilisé. |
...
|---|---|---|---|---|---|
| `HealthFacilityCreated` | `tenant.health-facility.created` | 1 | `HealthFacility.create()`, via `CreateHealthFacilityHandler` | `name: string`, `ownerUserId: string` (ADR-0008 §9, amendement 1 — **ajout additif**, `eventVersion` reste `1` : identifiant du `UserAccount` a l'origine du provisioning, jamais stocke sur `HealthFacility` lui-meme, uniquement une donnee de correlation portee par cet evenement) (`tenantId === aggregateId` : l'agrégat EST le tenant) | **Un seul consommateur** (resequencement F3, revue de sécurité indépendante de l'étape 10/13 — voir ADR-0008 §1/§4/§9) : `subscription.startTrialSubscriptionOnHealthFacilityCreated` (module `subscription`) — démarre l'essai gratuit STANDARD, lit désormais `ownerUserId` depuis le payload (validé par un schéma Zod de frontière) et le propage à `SubscriptionStarted`. **N'accorde plus `ADMIN_ETABLISSEMENT` directement** : ce consommateur n'existe plus sur ce canal (voir `SubscriptionStarted` ci-dessous, `identity.grantOwnerMembershipOnSubscriptionStarted`) — la chaîne est désormais strictement séquentielle, jamais deux étapes en parallèle sur ce déclencheur. |
| `FacilityConfigurationSeeded` | `tenant.facility-configuration-seeded` | 1 | `FacilitySettings.create()`, via `SeedFacilityConfigurationHandler` | `locale: string`, `timezone: string`, `currency: string`, `phoneCountryCode: string` (`aggregateId` = `FacilitySettingsId`, `tenantId` distinct — voir `FacilitySettings.ts`) | `tenant.completeProvisioningOnFacilityConfigurationSeeded` (module `tenant`, **émis et consommé par le même module**, ADR-0008 §11) — clôt la Saga en émettant `ProvisioningCompleted`. N'extrait aucun champ du payload, seul `envelope.tenantId` est utilisé. |
| `ProvisioningCompleted` | `tenant.provisioning.completed` | 1 | `FacilitySettings.completeProvisioning()`, via `CompleteProvisioningHandler` | *(aucun champ additionnel — signal de clôture minimal, ADR-0008 §11)* | **Aucun.** JAMAIS consulté par `TenantModuleBackedAccessChecker` (ADR-0008 §3/§11, non négociable — l'accès reste dérivé dynamiquement de `HealthFacility`/`Subscription`). Point d'extension futur possible côté `apps/web` (démarrage du parcours onboarding), pas encore implémenté. |

## Module `subscription`

| Événement | `eventType` | Version | Producteur (agrégat / commande) | Payload | Consommateur(s) Outbox actuel(s) |
|---|---|---|---|---|---|
| `SubscriptionStarted` | `subscription.subscription.started` | 1 | `Subscription.startTrial()`, via `StartTrialSubscriptionHandler` | `planId: string`, `trialEndsAt: string \| null`, `ownerUserId: string` (ADR-0008 §9, resequencement F3 — **ajout additif**, `eventVersion` reste `1` : identifiant du `UserAccount` à l'origine du provisioning, relu par `StartTrialSubscriptionOnHealthFacilityCreated` depuis `HealthFacilityCreated.ownerUserId` et propagé ici) | **Deux consommateurs**, sur le MÊME `eventType` (même pattern que `SaaSPaymentSucceeded`), sans dépendance causale entre eux : `notifications.sendWelcomeEmailOnSubscriptionStarted` (module `notifications`, étape 9/13) — email de bienvenue, n'extrait aucun champ du payload ; `identity.grantOwnerMembershipOnSubscriptionStarted` (module `identity`, ADR-0008 §1/§4/§9, resequencement F3, Phase 0 étape 10/13) — DEUXIÈME étape chorégraphiée de la Saga de provisioning, accorde `ADMIN_ETABLISSEMENT` à `ownerUserId`, lu depuis le payload validé par un schéma Zod de frontière. **Ex-`identity.grantOwnerMembershipOnHealthFacilityCreated`** (retiré du canal `tenant.health-facility.created`, voir ci-dessus) : la revue de sécurité indépendante de l'étape 10/13 (finding F3, Moyen) a constaté que l'ancien branchage en parallèle de `StartTrialSubscription` pouvait laisser `MembershipGranted`/`ProvisioningCompleted` précéder `SubscriptionStarted` — corrigé en rendant la chaîne strictement séquentielle. |
